...
class Weka:
    ...
    #
    def predict(self, modelName, x, arffName, legal, debug=False):
        ...
        if data.class_attribute.is_nominal:
            ...
            if pred not in legal:
                for i in range(1, len(sorted_dist)):
                    if sorted_dist[i] == north:
                        pred = data.class_attribute.value(0.0)
                        if pred in legal:
                            return pred
                    if sorted_dist[i] == south:
                        pred = data.class_attribute.value(1.0)
                        if pred in legal:
                            return pred
                    if sorted_dist[i] == west:
                        pred = data.class_attribute.value(2.0)
                        if pred in legal:
                            return pred
                    if sorted_dist[i] == east:
                        pred = data.class_attribute.value(3.0)
                        if pred in legal:
                            return pred
        if(debug):
